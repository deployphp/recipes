...
function getCpanel(){
    ...
    if(!in_array($stage, $allowIn)){
        throw new \RuntimeException(sprintf("Since it creates addon domains and databases, CPanel recipe is available only in the %s environments", implode($allowIn)));
    }
...
task('cpanel:createdb', function () {
    ...
    if (isset($addPrivilegesData['cpanelresult']['error'])){
        ...
        writeln('Succesfully added privileges!');
    }
});
